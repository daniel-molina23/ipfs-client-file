    ...
    if(!node){
        ...
        node.once('ready', () => {
            node.id().then((id) => {
                ...
                setInterval(refreshPeerList, 1000) // we check every second
                setInterval(sendFileList, 10000) //check every 10 secs
            })
...
function appendFile(name, hash, size, data){
    ...
    //created HTMl elements for corresponding parts
    ...
    const link = document.createElement('a') //anchor tag which links 2 objects together
    link.setAttribute('href', url) //value first then value
    link.setAttribute('download', name) //value first then value
    ...
    files.forEach((file) => {
        readFileContents(file)
            .then((buffer) => {
                ...
                },{ wrap: true, progress: updateProgress}, (error, filesAdded) => {
                    if(error){
...
function connectToPeer(event) {
    ...
    if(!multiaddr){
        return onError('No MultiAddress was Inserted')
    }
...
function refreshPeerList(){
    node.swarm.peers()
        .then((peers) => {
            const peersAsHTML = peers.reverse() //shows top to bottom
                .map((peer) => {
                    if(peer.addr){
